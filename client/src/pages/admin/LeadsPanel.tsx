...
export function LeadsPanel({ eventId, flowSlug = 'default' }: LeadsPanelProps) {
  ...
  const load = () => {
    setLoading(true)
    setError('')
    const req = eventId ? api.getEventLeads(eventId) : api.getFlowLeads(flowSlug)
    req
      .then(setLeads)
  ...
  useEffect(() => { load() }, [eventId, flowSlug])

  const exportCsv = async () => {
    try {
      if (eventId) await api.exportEventLeadsCsv(eventId)
      else await api.exportFlowLeadsCsv(flowSlug)
